...
class Obj(abc.ABC):
    ...
    def _setObjArrayValues(self, values: Optional[np.ndarray] = None) -> None:
        ...
        Performs sanity checks for the 'shape' and 'border_shape' parameters supplied when the class is created. The
        'shape' parameter should be tuple-like and composed of integers, formatted so that 'numpy.zeros' accepts it
        as an argument for the 'shape' parameter. The 'border_shape' should be formatted so that 'numpy.pad' accepts it
